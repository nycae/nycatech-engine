...
void Package::AddAsset(const String& file) { assets.Insert(file); }

void Package::Bundle(const String& file) {
  FileWritter outFile(file.c_str(), std::ios::binary);
  ZSTD_CCtx* ctx = ZSTD_createCCtx();

  Vector<char> outBuffer(ZSTD_compressBound(SizeBuffer));
  ...
  for (const auto& assetFile : assets) {
    ...
    Uint64 sizeCompressed = ZSTD_compressCCtx(
        ctx, outBuffer.Data(), outBuffer.Capacity(), data.data(), data.size(), ZSTD_defaultCLevel());
